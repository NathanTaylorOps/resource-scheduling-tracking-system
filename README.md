...
lib/domain/           Pure, framework-free business logic — scheduling, equipment-reservation conflicts,
                       compliance, certifications, maintenance hierarchy, forecasting, readiness scoring,
...
- **Crew requirements, equipment reservations, permits, and field documentation are all read-only for this pass.** The staffing plan and forward equipment bookings, permits and inspections, daily logs, and toolbox talks are fully modeled, seeded with realistic (synthetic) data, and feeding readiness where that's appropriate (the first three) or displayed for reference (the last two) — but there's no UI yet to create or edit any of them from the app itself; today they exist because the seed data populates them, the same way equipment usage logging works below. Building those forms is additive to the existing structure, not a redesign — the pattern (a page, a form, a route handler) is already established by the equipment check-in/check-out flow.
