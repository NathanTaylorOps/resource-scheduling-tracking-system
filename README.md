...
Written down here rather than left for someone to discover. None of what follows is a defect: each item below is either a scope boundary chosen on purpose for a v1 — and explained inline, not just asserted — or an acknowledged gap with a clear next step already thought through. A build that claims to have no edges either hasn't been pushed hard enough to find them, or isn't saying. This is the honest version:
