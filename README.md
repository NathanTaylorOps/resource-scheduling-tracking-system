...
- **Jobs, crew, equipment, subcontractor firms, and assignments can be created but not yet edited or deleted.** A typo in a job's name or a worker's hire date has no in-app fix once it's saved. The six within-a-record write paths (crew requirements, equipment reservations, permits and inspections, daily logs, toolbox talks, subcontractor license/COI) were always full add/remove; edit and delete for these five is the natural next slice of this same work, not a different kind of feature.
