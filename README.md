...
- **Jobs, workers, equipment, subcontractor firms, and crew assignments are fixture data for this pass.** The write paths built so far update records that live inside an existing job, equipment asset, or subcontractor firm — crew requirements, equipment reservations, permits and inspections, daily logs, toolbox talks, and a subcontractor's license and COI. Creating a new job, worker, equipment record, or subcontractor firm — or assigning a worker to a job in the first place — still only happens through the seed data (`prisma/seed.ts`). `canAssignWorker` (`lib/domain/certifications.ts`) already implements the certification hard-stop described above, ready for the day an assignment-creation flow calls it; building these six record types out is the natural next slice of this same write-path work, not a different kind of feature.
