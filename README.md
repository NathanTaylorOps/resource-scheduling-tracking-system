...
A later pass added a handful of features aimed at what a bigger GC — more jobs, more subs, a real safety program, public-money work — actually needs on top of the operational core above. They're real, usable features built on this app's existing per-visitor demo architecture, not a claim that this is now a certified-payroll system, a compliance-department replacement, or a real access-control layer.

- **The "viewing as" role switcher** (`components/RoleSwitcher.tsx`, `lib/role.ts`, `lib/actor.ts`) lets you pick a role — Executive, PM, Superintendent, Safety Director, Subcontractor, Field Worker — and see the app hide or show controls the way that role's permissions say it should. This is a UI-only demonstration of the RBAC *concept*, not real authentication or access control: every check is enforced only in rendering, never in an API route, because this app has no login to verify who's actually asking (see the Scope section above). If this app ever got real auth, `lib/role.ts`'s permission matrix is what that auth would need to enforce server-side; today it only enforces what's rendered. A one-line `RoleContextBanner` on the job and worker pages names, out loud, what the current role can and can't do there — a hidden Pencil icon or a missing dollar column is easy to miss entirely, and the point of the demo is that a reviewer sees the restriction, not that they go hunting for it.
- **The audit trail** (`lib/audit.ts`, `AuditLogEntry`) logs who changed what on the handful of fields that actually get disputed later — a permit's status, an inspection outcome, a lien waiver's status, a safety incident's status — not every mutation in the app. It's deliberately not a field-by-field diff engine, and attribution is only as trustworthy as the "viewing as" identity that produced it, since there's no real login behind it — an honest record of what this app can actually vouch for, not a claim of forensic-grade provenance. It's also not a write-only log: each job page has a read-only "Recent activity" card pulling every audit entry attached to that job's own permits, inspections, lien waivers, and safety incidents, so the trail is something you can actually look at, not just something being quietly written.
- **Lien waivers** (`LienWaiver`) track which waiver — conditional/unconditional, progress/final — is outstanding against which pay period for which sub, the single biggest gap two independent enterprise-scale reviews of this app converged on unprompted: a missing waiver blocks the GC's own pay application to the owner. It's a tracking record, not a document-generation or e-signature system.
- **Safety incidents** (`SafetyIncident`) log injuries, property damage, environmental events, and near-misses as a first-class type of their own — deliberately not a lesser severity of "injury," since a near-miss is exactly the leading indicator a real safety program tracks trends against. This is incident tracking, not OSHA 300-series recordkeeping (Forms 300/300A/301), which stays out of scope for the same reasons toolbox talks already are (see Scope, above).
- **Job hazard analyses** (`JobHazardAnalysis`) record a task, its identified hazards, and the controls in place for it — the pre-task planning document a real safety program expects on file before higher-risk work starts, not an auto-generated risk assessment. A job flagged weather-sensitive or conditional gets a lightweight nudge on its JHA card if nothing on file reads like it covers weather exposure (cold/heat stress, wet footing, wind on elevated work) — a suggestion, not a gate, and it goes away the moment a JHA mentions any of those terms.
- **Worker screenings** (`WorkerScreening`) record drug test and background check results per worker, append-only — a new test is a new row, never an edit to an old one, so the history stays intact. It's a result log, not a lab or background-check vendor integration.
- **Certified payroll entries** (`CertifiedPayrollEntry`) record hours, classification, and rate by worker by week for jobs that carry `certifiedPayrollRequired` — a private residential job never needs one; a job touching public money or Davis-Bacon-style prevailing-wage terms does. This is **not** a full payroll system: no tax withholding, no benefits administration, and no WH-347 export — it's a record of the numbers a certified payroll report would be built from, not the report itself.
- **The portfolio rollup** (`/portfolio`) groups every job by `Job.division` and shows each division's job count and readiness mix — for a COO or regional Project Executive overseeing several jobs at once, a view the single flat jobs list can't give them. A single-office company (like the fictional Coastwood Builders this demo is seeded around) has no real use for it, which is why `division` is nullable and unset jobs still show up, grouped as "Unassigned," rather than silently dropped.
